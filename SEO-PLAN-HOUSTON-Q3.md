...
**2026-06-12 — Houston service pages + perf pass. `pnpm build` GREEN, 168 pages.**
- **Missing Houston service pages (owner catch):** the engine generated `{service}-{neighborhood}` for all 25 neighborhoods but not for "houston" itself. Added a `houston` profile → now `/recurring-house-cleaning-houston`, `/deep-cleaning-houston`, `/move-in-move-out-houston` exist (high-intent city-level queries). Excluded the `house-cleaning-houston` combo from the engine (it has its own enriched explicit page). +3 pages.
- **Perf (PageSpeed mobile 57; LCP 8.9s is the killer, FCP 1.7s, CLS 0, TBT 580ms):** mobile LCP is text (hero image is `hidden md:block`), delayed by main-thread JS — so the levers are the report's JS items. Added `experimental.optimizePackageImports` (lucide, framer-motion, embla, radix → cuts "unused JavaScript" 251 KiB) + modern `browserslist` (drops "legacy JavaScript" 33 KiB). Earlier: `next/image` on 121 location pages (image delivery + CLS). _Note: heading font-family name mismatch ('allRoundGothic' vs "All Round Gothic") means the brand heading font isn't downloaded — not a perf issue, but a cosmetic bug for later._
- **Could not measure the delta** — Google's keyless PSI API quota is exhausted today. Re-run PageSpeed after deploy; if LCP is still high, the next lever is deferring/replacing GTM+PostHog+Tally main-thread work (carefully, conversion-critical) or a map facade.

**2026-06-12 — Reviews carousel + house-cleaning-houston enhancements. `pnpm build` GREEN, 168 pages.**
